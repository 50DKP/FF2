...
public Action:FF2_OnAbility2(index,const String:plugin_name[],const String:ability_name[],action)
{
	...
	//slot 0
	else if(!strcmp(ability_name,"rage_cloneattack"))  //Seeldier's Clone Attack
	{
		Rage_CloneAttack(ability_name,index);
	}
	else if(!strcmp(ability_name,"rage_tradespam"))  //Demopan's Trade Spam
	{
		Rage_Timer_Demopan_Trade_Spam(INVALID_HANDLE,1);
	}
	else if(!strcmp(ability_name,"rage_cbs_bowrage"))  //CBS's Bow Rage
	{
		Rage_UseBow(index);
	}
	else if(!strcmp(ability_name,"rage_explosive_dance"))  //Seeman's explosive dance
	{
...
public Action:Rage_Timer_Explosive_DanceB(Handle:hTimer,any:index)
{
	...
	if(count<=35 && IsPlayerAlive(Boss))
	{
		...
		for(new i=0; i<5; i++)
		{
			new proj=CreateEntityByName("env_explosion");   
...
public Action:Rage_Timer_NinjaAttacks(Handle:hTimer,Handle:data)
{
	...
	if(target>0)
	{
		...
		if(GetVectorDistance(pos,pos2)<1500 && target!=oldtarget)  //Chris:  Don't let Ninja Spy/etc. spam their slow-mo attacks at the same person.
		{
